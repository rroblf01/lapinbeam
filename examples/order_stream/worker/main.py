...
actually run orders (see order.py). Never dials out — `app` connects to
*this* node, and pushes are sent back over that same connection using the
`reply_node` id carried in each dispatch message, so this node needs no
address for `app` configured anywhere.
"""
